...
private:
  ...
  string _phase; // Represent the phase if _sim_type == 1 (SOLID, LIQUID or GAS)
  string _autocorr; // If it must calculate autocorrelation or not
...
public: // Function declarations
  ...
  void measure(int blk);             // Measure properties of the system
  ...
  void Increase_temp(double delta); // Increase the nominative temperature
  void initialize_plot_files(); // Initialize the files with plot data
  void averages_plot(int blk, int temp_step); // Compute averages of properties for plot files
