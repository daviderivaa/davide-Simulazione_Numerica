...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double p = 1./(1.+exp(-2.*_beta*(_J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin())+_H)));
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream input_2("../INPUT/input.dat"); // Start reading ../INPUT/input.dat
  string property;
  double delta;
  while ( !input_2.eof() ){
    input_2 >> property;
  ...
  ofstream coutf;
  coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/output.dat");
  while ( !input.eof() ){
    input >> property;
    if( property == "SIMULATION_TYPE" ){
      input >> _sim_type;
  ...
  ofstream couta("../OUTPUT/" + _phase + "/" + _autocorr + "/acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( (property == "POTENTIAL_ENERGY") and (_H == 0.0) ){
        ofstream coutp("../OUTPUT/" + _phase + "/" + _autocorr + "/potential_energy.dat");
        ...
        index_property++;
        _vtail = 0.0;
      } else if( (property == "KINETIC_ENERGY") and (_H == 0.0) ){
        ofstream coutk("../OUTPUT/" + _phase + "/" + _autocorr + "/kinetic_energy.dat");
        ...
        index_property++;
      } else if( (property == "TOTAL_ENERGY") and (_H == 0.0) ){
        ofstream coutt("../OUTPUT/" + _phase + "/" + _autocorr + "/total_energy.dat");
        ...
        index_property++;
      } else if( (property == "TEMPERATURE") and (_H == 0.0) ){
        ofstream coutte("../OUTPUT/" + _phase + "/" + _autocorr + "/temperature.dat");
        ...
        index_property++;
      } else if( (property == "PRESSURE") and (_H == 0.0) ){
        ofstream coutpr("../OUTPUT/" + _phase + "/" + _autocorr + "/pressure.dat");
        ...
        ofstream coutgr("../OUTPUT/" + _phase + "/" + _autocorr + "/gofr.dat");
      ...
      } else if( (property == "MAGNETIZATION") and (_H > 0.0) ){
        ofstream coutpr("../OUTPUT/" + _phase + "/" + _autocorr + "/magnetization.dat");
        ...
        index_property++;
      } else if( (property == "SPECIFIC_HEAT") and (_H == 0.0) ){
        ofstream coutpr("../OUTPUT/" + _phase + "/" + _autocorr + "/specific_heat.dat");
        ...
        index_property++;
      } else if( (property == "SUSCEPTIBILITY") and (_H == 0.0) ){
        ofstream coutpr("../OUTPUT/" + _phase + "/" + _autocorr + "/susceptibility.dat");
        ...
        index_property++;
      } else if( (property == "ENDPROPERTIES") and (_H == 0.0) ){
        ofstream coutf;
        coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/output.dat",ios::app);
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: measure(int blk){ // Measure properties
  _measurement.zeros();
  ...
  double cv_temp=0.0; // temporary accumulator for specific heat
  double virial=0.0;
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        if (_measure_gofr and (dr < (_halfside.min()))) {
          _measurement[_index_gofr+static_cast<int>(floor(dr/_bin_size))] += 2;
        }
        if(dr < _r_cut){
          // POTENTIAL ENERGY
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    _measurement(_index_penergy) = _vtail + 4.0*penergy_temp/double(_npart);
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv and _measure_temp and _measure_tenergy){
    double s_i, s_j;
    for (int i=0; i<_npart; i++){
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i+1)).getspin());
      cv_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
    }
    _measurement(_index_cv) = pow(cv_temp,2);
  } 
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi and _measure_temp){
    for (int i=0; i<_npart; i++){
      _measurement(_index_chi) += double(_particle(i).getspin());
    }
    _measurement(_index_chi) = _beta*pow(_measurement(_index_chi),2)/double(_npart);
  }
  // GOFR /////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    for (int i=_index_gofr;i<_index_gofr+_n_bins;i++) {
      _measurement(i) /= ((_rho*double(_npart)*(4./3.)* M_PI*(pow((_bin_size*(i+1)),3)-pow((_bin_size*i),3))));
    }
  }
...
void System :: averages(int blk){
  ...
  if (_measure_cv){
    ...
    _global_av2(_index_cv) -= _average(_index_cv) * _average(_index_cv); // % -> element-wise multiplication
    _average(_index_cv) = (pow(_beta,2)*((_block_av(_index_cv)/double(_nsteps))-pow(((_block_av(_index_tenergy)*double(_npart))/double(_nsteps)),2)))/double(_npart);
    _global_av(_index_cv)  += _average(_index_cv);
    _global_av2(_index_cv) += _average(_index_cv) * _average(_index_cv); // % -> element-wise multiplication
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/specific_heat.dat",ios::app);
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/temperature.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/pressure.dat",ios::app);
  ...
  if (_measure_gofr and (blk == _nblocks)){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/gofr.dat",ios::app);
    for (int i=_index_gofr;i<_index_gofr+_n_bins;i++)  {
    sum_average = _global_av(i);
    sum_ave2 = _global_av2(i);
    coutf << setw(12) << _bin_size*(i-_index_gofr+1)
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/magnetization.dat",ios::app);
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/susceptibility.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/acceptance.dat",ios::app);
...
void System :: Increase_temp(double delta){
  ifstream file("../INPUT/input.dat");
    if (!file.is_open()) {
        cerr << "Impossibile aprire il file: " << "../INPUT/input.dat" << std::endl;
        return;
    }
    ...
    while (getline(file, line)) {
        if (line.find("TEMP") == 0) {
            ...
            string temp_tag;
            iss >> temp_tag >> temp_value;
            
            temp_value -= delta;
            updated_content << "TEMP " << temp_value << endl;
        } else {
    ...
    ofstream output_file("../INPUT/input.dat");
    if (!output_file.is_open()) {
        cerr << "Impossibile scrivere nel file: " << "../INPUT/input.dat" << endl;
        return;
    }

    output_file << updated_content.str();

    output_file.close();
}

void System :: initialize_plot_files(){ // Initialize the files with plot data
  ...
  if (input.is_open()){
    if (_sim_type == 2) {
      while ( !input.eof() ){
        input >> property;
        if( (property == "TOTAL_ENERGY") and (_H == 0.0) ){
          ofstream coutt_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/total_energy_MTR.dat");
          coutt_2 << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
          coutt_2.close();
        } else if( (property == "MAGNETIZATION") and (_H > 0.0) ){
          ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/magnetization_MTR.dat");
          coutpr_2 << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
          coutpr_2.close();
        } else if( (property == "SPECIFIC_HEAT") and (_H == 0.0) ){
          ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/specific_heat_MTR.dat");
          coutpr_2 << "#     BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
          coutpr_2.close();
        } else if( (property == "SUSCEPTIBILITY") and (_H == 0.0) ){
          ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/susceptibility_MTR.dat");
          coutpr_2 << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
          coutpr_2.close();
        }
      }
    } else if (_sim_type == 3) {
      ...
      input >> property;
      if( (property == "TOTAL_ENERGY") and (_H == 0.0) ){
        ofstream coutt_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/total_energy_GIBBS.dat");
        coutt_2 << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        coutt_2.close();
      } else if((property == "MAGNETIZATION") and (_H > 0.0)){
        ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/magnetization_GIBBS.dat");
        coutpr_2 << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutpr_2.close();
      } else if( (property == "SPECIFIC_HEAT") and (_H == 0.0) ){
        ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/specific_heat_GIBBS.dat");
        coutpr_2 << "#     BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
        coutpr_2.close();
      } else if( (property == "SUSCEPTIBILITY") and (_H == 0.0) ){
        ofstream coutpr_2("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/susceptibility_GIBBS.dat");
        coutpr_2 << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutpr_2.close();
        }
      }
...
void System :: averages_plot(int blk, int temp_step){
  ...
  if (_sim_type == 2) {
    ...
    if (_measure_cv){
      ...
      _global_av(_index_cv)  += _average(_index_cv);
      _global_av2(_index_cv) += _average(_index_cv) * _average(_index_cv); // % -> element-wise multiplication
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/specific_heat_MTR.dat",ios::app);
      average  = _average(_index_cv);
      sum_average = _global_av(_index_cv);
      sum_ave2 = _global_av2(_index_cv);
      coutf << setw(12) << temp_step
            << setw(12) << average
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf.close();
  ...
  if (_sim_type == 3) {
    ...
    if (_measure_cv){
      ...
      _global_av(_index_cv)  += _average(_index_cv);
      _global_av2(_index_cv) += _average(_index_cv) * _average(_index_cv); // % -> element-wise multiplication
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/specific_heat_GIBBS.dat",ios::app);
      average  = _average(_index_cv);
      sum_average = _global_av(_index_cv);
      sum_ave2 = _global_av2(_index_cv);
      coutf << setw(12) << temp_step
  ...
  if (_sim_type == 2) {
    // TOTAL ENERGY //////////////////////////////////////////////////////////////
    if (_measure_tenergy){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/total_energy_MTR.dat",ios::app);
      average  = _average(_index_tenergy);
      sum_average = _global_av(_index_tenergy);
      sum_ave2 = _global_av2(_index_tenergy);
      coutf << setw(12) << temp_step
    ...
    // MAGNETIZATION /////////////////////////////////////////////////////////////
    if (_measure_magnet){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/magnetization_MTR.dat",ios::app);
      average  = _average(_index_magnet);
      sum_average = _global_av(_index_magnet);
      sum_ave2 = _global_av2(_index_magnet);
      coutf << setw(12) << temp_step
    ...
    // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
    if (_measure_chi){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/susceptibility_MTR.dat",ios::app);
      average  = _average(_index_chi);
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf << setw(12) << temp_step
    ...
    // TOTAL ENERGY //////////////////////////////////////////////////////////////
    if (_measure_tenergy){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/total_energy_GIBBS.dat",ios::app);
      average  = _average(_index_tenergy);
      sum_average = _global_av(_index_tenergy);
      sum_ave2 = _global_av2(_index_tenergy);
      coutf << setw(12) << temp_step
    ...
    // MAGNETIZATION /////////////////////////////////////////////////////////////
    if (_measure_magnet){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "/PLOT/magnetization_GIBBS.dat",ios::app);
      average  = _average(_index_magnet);
      sum_average = _global_av(_index_magnet);
      sum_ave2 = _global_av2(_index_magnet);
      coutf << setw(12) << temp_step
    ...
    // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
    if (_measure_chi){
      coutf.open("../OUTPUT/" + _phase + "/" + _autocorr + "v/PLOT/susceptibility_GIBBS.dat",ios::app);
      average  = _average(_index_chi);
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
      coutf << setw(12) << temp_step
